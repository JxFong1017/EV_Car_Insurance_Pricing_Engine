from app.models.schemas import PackagingBreakdown

RIDER_CONFIG = {
    "mobility": {
        "label": "Battery Downtime Mobility Allowance (RM 4,000 limit)",
        "amount": 358.14,
    },
    "ncd_shield": {
        "label": "NCD Shield / 1-Tier Step-Down Protector",
        "amount": 0.0,  # Applied as % loading on base gross
    },
    "cyber": {
        "label": "Cyber Security V2G Protection Rider",
        "amount": 0.0,  # Applied as % loading on base gross
    },
}

TIER_1_ITEMS = [
    {"label": "Portable Charging Cable Cover (Up to RM 5,000)", "cost": "Included in RM 15 Fee", "amount": 0.0},
    {"label": "Public Station Inconvenience / Compassionate Cover (Up to RM 5,000)", "cost": "Included in RM 15 Fee", "amount": 0.0},
    {"label": "24/7 Flat Battery Towing (Nearest charger / Home)", "cost": "Included in RM 15 Fee", "amount": 0.0},
]

TIER_2_ITEMS = [
    {"label": "All Tier 1 Benefits", "cost": "Included in RM 93 Fee", "amount": 0.0},
    {"label": "Loss or Damage to Home Wall Charger (Up to RM 20,000)", "cost": "Included in RM 93 Fee", "amount": 0.0},
    {"label": "Personal Liability due to Home Wall Charger (Up to RM 50,000)", "cost": "Included in RM 93 Fee", "amount": 0.0},
    {"label": "Charger Fire Cleanup & Site Restoration Allowance (RM 3,000)", "cost": "Included in RM 93 Fee", "amount": 0.0},
]

TIER_3_ITEMS = [
    {"label": "All Tier 1 & Tier 2 Benefits", "cost": "Included in RM 150 Fee", "amount": 0.0},
    {"label": "Compassionate Cover for Home Damage due to Home Wall Charger (Up to RM 5,000)", "cost": "Included in RM 150 Fee", "amount": 0.0},
    {"label": "24/7 Flat Battery Towing Upgrade (Unlimited mileage)", "cost": "Included in RM 150 Fee", "amount": 0.0},
]


def calculate_packaging(
    tier_mode: str,
    selected_riders: list[str],
    smart_grid_enrolled: bool,
    base_motor_gross: float,
    sum_insured: float,
) -> PackagingBreakdown:
    tier_loading_factor = 1.0
    smart_grid_discount = 1.0
    final_base_gross = base_motor_gross
    package_premium = 0.0
    rider_items: list[dict] = []

    def apply_ecosystem_addons() -> None:
        nonlocal package_premium
        if "ncd_shield" in selected_riders:
            ncd_prem = base_motor_gross * 0.1000
            package_premium += ncd_prem
            rider_items.append({
                "label": RIDER_CONFIG["ncd_shield"]["label"],
                "cost": "+10% Base Gross",
                "amount": round(ncd_prem, 2),
            })

        if "cyber" in selected_riders:
            cyb_prem = base_motor_gross * 0.1000
            package_premium += cyb_prem
            rider_items.append({
                "label": RIDER_CONFIG["cyber"]["label"],
                "cost": "+10% Base Gross",
                "amount": round(cyb_prem, 2),
            })

        if "mobility" in selected_riders:
            mobility_premium = RIDER_CONFIG["mobility"]["amount"]
            package_premium += mobility_premium
            rider_items.append({
                "label": RIDER_CONFIG["mobility"]["label"],
                "cost": f"RM {mobility_premium:.2f}",
                "amount": mobility_premium,
            })

    if tier_mode == "1":
        tier_name = "Tier 1: EV Essential (High-rise / Condo residents)"
        package_premium = 15.00
        rider_items = list(TIER_1_ITEMS)
        apply_ecosystem_addons()

    elif tier_mode == "2":
        tier_name = "Tier 2: EV Advanced (Landed property owners)"
        package_premium = 93.00
        rider_items = list(TIER_2_ITEMS)
        apply_ecosystem_addons()

    elif tier_mode == "3":
        tier_name = "Tier 3: EV Premium (Full ecosystem coverage)"
        package_premium = 150.00
        rider_items = list(TIER_3_ITEMS)
        apply_ecosystem_addons()

    else:  # basic — standalone riders only
        tier_name = "Basic Comprehensive Cover (Standalone)"
        apply_ecosystem_addons()

    tier_loading_factor = round(final_base_gross / base_motor_gross, 4) if base_motor_gross else 1.0

    return PackagingBreakdown(
        tier_name=tier_name,
        tier_loading_factor=tier_loading_factor,
        smart_grid_discount=smart_grid_discount,
        final_base_gross=final_base_gross,
        package_premium=round(package_premium, 2),
        rider_items=rider_items,
    )
